...
$jsonFilePath = "C:\Development\seeseeportal\games.json"
...
foreach ($game in $gamesData) {
    try {
        ...
        if (-not $gameFolderPath) {
            Write-Host "  ⚠ Skipped: Game missing 'folder' property" -ForegroundColor Yellow
            $errorCount++
            continue
        }
        ...
        if (-not (Test-Path $gameFolderPath)) {
            Write-Host "  ⚠ Skipped: Folder not found - $gameFolderPath" -ForegroundColor Yellow
            $errorCount++
            continue
        }
        ...
        Write-Host "  ✗ Error processing $($game.title): $_" -ForegroundColor Red
        $errorCount++
    }
}
